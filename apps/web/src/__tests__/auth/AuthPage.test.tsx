...
vi.mock('@/features/auth/AuthBrand', () => ({ default: () => <h1>AssetPredict</h1> }));
vi.mock('@/features/auth/AuthTabs', () => ({ default: () => null }));
vi.mock('@/features/auth/SignUpForm', () => ({
    default: () => <form><button type="submit">Sign Up</button></form>,
}));
vi.mock('@/features/auth/SignInForm', () => ({
    default: () => <form><button type="submit">Sign In</button></form>,
}));
...
describe('AuthPage', () => {
    ...
    it('contains AuthBrand and toggle link', () => {
        render(<AuthPage />);
        expect(screen.getByRole('heading', { name: 'AssetPredict' })).toBeInTheDocument();
