...
export default function RecentAssetsBar({ state }: RecentAssetsBarProps) {
    const renderContent = () => {
        if (state === 'loading') return Array(3).fill(null).map((_, i) => <Pill key={i} isSkeleton />); // Добавлен null для типа
        if (state === 'empty') return <p className="text-gray-500">No assets available</p>;

        return (
            <>
                {mockAssets.map((asset: Asset) => (
                    <Pill key={asset.symbol} label={`${asset.symbol} ${asset.price}`} variant="unselected"/>
