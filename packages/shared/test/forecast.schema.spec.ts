...
import {
  zForecastCreateReq,
  zForecastDetailRes,
} from '../src/schemas/forecast.schema.js';
...
describe('zForecastCreateReq', () => {
  ...
  it('должен отклонять невалидный timeframe', () => {
    ...
    if (!result.success) {
      expect(
        result.error.errors.some((e) => e.path.includes('timeframe')),
      ).toBe(true);
    }
  });
  ...
  it('должен отклонять horizon > MAX_HORIZON', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const horizonError = result.error.errors.find((e) =>
        e.path.includes('horizon'),
      );
...
describe('zForecastDetailRes', () => {
  ...
  it('должен принимать валидный объект с factors и metrics', () => {
    const valid = {
      ...
      factors: [{ name: 'volume', impact: 0.5, shap: 0.3, conf: 0.8 }],
      metrics: {
        mae: 2.5,
        mape: 1.2,
        coverage: 0.95,
  ...
  it('должен проверять, что series.t.length === series.p50.length', () => {
    ...
    if (!result.success) {
      const error = result.error.errors.find((e) =>
        e.message.includes('same length'),
      );
  ...
  it('должен проверять логику перцентилей: p10 <= p50 <= p90', () => {
    ...
    if (!result.success) {
      const error = result.error.errors.find((e) =>
        e.message.includes('p10 <= p50 <= p90'),
      );
      expect(error).toBeDefined();
    }
  });
